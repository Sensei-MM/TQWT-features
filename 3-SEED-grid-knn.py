...
def band_accuracy(data):
    ...
    tuned_parameters = [{'weights':['uniform'], 'n_neighbors':[i for i in range(1,20)]}]
    scores = [ 'accuracy']
    for score in scores:
        print("# Tuning hyper-parameters for %s" % score)
        print()
        clf = GridSearchCV(knn_clf, tuned_parameters)
...
if __name__ == '__main__':
    ...
    band = 8
    knn_linear_acc = np.zeros((len(people_name), band))
    for sub in range(len(people_name)):
        ...
        for b in range(band):
            ...
            knn_linear_acc[sub,b] = band_acc
    dataframe = DataFrame(knn_linear_acc)
    with pd.ExcelWriter('G:\Paper-EEG Emotion\SEED-tqwt2-8band-q1r3j7-2\Analysis-Trditional\\knn-de0.5-grid.xlsx') as writer:
        dataframe.to_excel(writer, sheet_name='de-q1r3j7-8band-knn-day1')